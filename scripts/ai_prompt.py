...
# ---------------------------------------------------------------------------
# §5 explain モード（1ハンド詳細解説）

EXPLAIN_SYSTEM_PROMPT = """あなたはGTOベースのポーカーコーチです。1ハンドを詳細に解説してください。

## 出力形式
フリーテキスト（JSON不要）。400〜1200文字。以下の必須セクションを段落で展開する:
1. 均衡レンジとHeroのポジション（OOP/IP）
2. GTO数学的観点（MDF/必要成功率/バリューターゲット）
3. 相手レンジの変化と読み
4. 相手への搾取戦略（bet/raise/fold等のアクション名で具体的に）
5. 代替ライン

""" + QUALITY_RULES_BLOCK


def build_explain_prompt(hand: dict) -> tuple[str, str]:
    """explainモードの (system_prompt, user_prompt) を返す。"""
    user = (
        "以下のハンドを詳細に解説してください。\n\n"
        + serialize_hand_context(1, hand)
    )
