...
- **V1パラメータ**: 中間帯の幅は 必要エクイティ ±0.15（`GTO_JUDGE_MARGIN`）。
  ハンド強度 = 全ヴィラン2枚コンボに対する treys 勝率近似（tie は 0.5 勝）。
  treys未導入・ボード/ホールカード欠落など**データ不足時も判定困難に倒す**
...
- **2026-07-02:** `scripts/classify.py` として実装（Status: ✅）。GTOスコア判定V1の
  パラメータ（中間帯±0.15・データ不足→判定困難）を§3に明文化。
  §3のフローが定義しないチェックダウン到達ショーダウンは、勝利→`value_success`
  （ライン定義 blue=ショーダウン勝利 を優先）、敗北→`call_lost`（判定困難）に倒す。
