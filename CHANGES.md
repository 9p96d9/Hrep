...
## 6. コーディング開始（2026-07-04・Flaskスキャフォールド）

- Cloud Run用アプリ本体を **Flask + gunicorn** で実装開始（infra.md旧記載のFastAPI + uvicornから変更・docs更新済み）
- `scripts/`（純粋関数）: `hand_converter.annotate_hand`（11カテゴリ分類・GTO数学・スコア）/ `ai_prompt` / `ai_validator`（本番受入ゲート兼テスト対象）
- `main.py` + `services/`: /health・/api/hand-capture・/api/analyze（SSE・BYOK）・セッション/結果画面
- テスト42件（gto_math / classify / ai_output）— GTO- からのコード移植はせず、specs/ の受入基準から新規実装
- GTO判定はV1ヒューリスティック（treys強度 vs 必要エクイティ・マージン±0.15・迷ったらwarn）

## 未完了（次のステップ）

- [x] hooks採否の決定 → **採用・実装・動作テスト済み**（`.claude/hooks/` 2本）
- [x] コーディング開始 → Flaskスキャフォールド完了（§6）。classify/gto_math ✅・ai_analysis 🔄
- [ ] GCPプロジェクトのCLI初期設定（`docs/infra.md` の「コンソールレス運用方針」参照）+ GitHub secret `GCP_SA_KEY`
- [ ] Chrome拡張（`extension/`）の実装（docs/features/extension.md）
- [ ] 実AIキーでの detail_street 出力品質検証（→ ai_analysis を ✅ に）
