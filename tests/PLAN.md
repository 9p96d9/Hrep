...
対象: `select_improve_chances`（specs/gto_math.md §4b）

```
test_improve_chances_street_priority_over_pot   → riverがturnより先
test_improve_chances_pot_desc_within_street     → 同ストリートはポット降順
test_improve_chances_excludes_non_target_categories → bluff_failed/warn/grayは対象外
test_improve_chances_max_three                  → 上位3件のみ
test_improve_chances_zero_candidates_returns_empty_list → 空リスト
```
...
test_decision_street_and_pot_in_output          → decision_street / decision_pot_bb
...
test_explain_system_prompt_has_required_sections → §5必須セクション・400〜1200文字
test_quality_rules_shared_across_modes    → §6品質ルールを両モードで共有
test_explain_user_prompt_quotes_gto_math_verbatim → [GTO数学]そのまま引用
```
